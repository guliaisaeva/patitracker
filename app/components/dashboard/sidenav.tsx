...
import Link from "next/link";
import NavLinks from "@/app/components/dashboard/nav-links";
import { PowerIcon, XMarkIcon } from "@heroicons/react/24/outline"; // Import Menu and X icons
...
export default function SideNav() {
  ...
  const router = useRouter();
  const { t } = useTranslation();
  const [menuOpen, setMenuOpen] = useState(false); // State to control burger menu
