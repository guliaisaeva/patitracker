...
import { getPetDetail, PetType, selectLanguages,fetchLanguages, selectPetDetail ,updatePetType} from '@/lib/features/pet/petTypesSlice';
...
export default function EditPetsForm({PetTypeId }: { PetTypeId: number}) {
  ...
  useEffect(() => {
    if (selectedPetTypeById) {
      ...
      setLanguageText(getLanguageText(selectedPetTypeById.languageId));
    }
  }, [selectedPetTypeById]);

  useEffect(() => {
    if (formState) {
      dispatch(updatePetType(formState));
    }
  }, [dispatch, formState]);

  const getLanguageText = (languageId: number): string => {
    switch (languageId) {
      case 1:
        return 'Turkish';
      case 2:
  ...
  const validateFormState = (formState: PetType): boolean => {
    ...
    const handleLanguageChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      setFormState((prevState) =>
        prevState ? { ...prevState, languageId: parseInt(value) } : null
      );
    };
